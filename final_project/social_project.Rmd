...
```{r}
media <- read.csv('C:/Users/Admin/Desktop/media_new.csv', row.names=1)
str(media)
```
...
### 1. Based on the given variables, can we classify if the student is facing problem while falling asleep or not, which can cause health issues because of social media usage.
### 2. Based on the given variables, can we predict if the student is facing problem while falling asleep or not, which can cause health issues because of social media usage.
...
```{r}
correlation_matrix <- cor(media[,1:11])
correlation_matrix

corrplot(cor(media), type = "upper", method = "color")
```
...
```{r}
media_pca <- prcomp(media[,1:8],scale=TRUE) 
media_pca

summary(media_pca)

fviz_eig(media_pca, addlabels = TRUE)
```
...
```{r}
fviz_pca_var(media_pca,col.var = "cos2",
...
```{r}
fit.pc <- principal(media[,1:8], nfactors=4, rotate="varimax") 
fit.pc
...
#### LinkedIn is used for professional purposes. Snapchat is used for chatting and Instagram is used for posting photos and videos. 
...
dist.media <- dist(matstd_sm, method="euclidean")
...
Actual <- ifelse(media$Trouble_falling_asleep == 1, "Trouble Sleep", "No trouble in sleep")
...
```{r}
wdbc <-read.csv('C:/Users/Admin/Desktop/media_new.csv')
